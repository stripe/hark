...
func TestReadAllOrFail_FindsNestedFiles(t *testing.T) {
	...
	results, err := ReadAllOrFail(context.Background(), fs, "/root", ReadOptions{Workers: 2})
...
func TestReadAllOrFail_IgnoresNonChangefiles(t *testing.T) {
	fs := afero.NewMemMapFs()
...
func TestReadAllOrFail_EmptyDirectory(t *testing.T) {
	fs := afero.NewMemMapFs()
	require.NoError(t, fs.MkdirAll("/empty", 0755))

	results, err := ReadAllOrFail(context.Background(), fs, "/empty", ReadOptions{Workers: 2})
	require.NoError(t, err)
	assert.Empty(t, results)
}

func TestReadAllOrFail_InvalidFile(t *testing.T) {
	fs := afero.NewMemMapFs()

	require.NoError(t, afero.WriteFile(fs, "/root/bad.change.md", []byte("no frontmatter here"), 0644))

	_, err := ReadAllOrFail(context.Background(), fs, "/root", ReadOptions{Workers: 2})
	require.Error(t, err)
}

func TestReadAllOrFail_ManyFiles(t *testing.T) {
	fs := afero.NewMemMapFs()
	...
	results, err := ReadAllOrFail(context.Background(), fs, "/root", ReadOptions{Workers: 4})
	require.NoError(t, err)

	assert.Len(t, results, 100)
}

func TestReadAllOrFail_SortedPathOrder(t *testing.T) {
	fs := afero.NewMemMapFs()
	...
	results, err := ReadAllOrFail(context.Background(), fs, "/root", ReadOptions{Workers: 3})
	require.NoError(t, err)
...
func TestReadAllOrFail_DefaultWorkers(t *testing.T) {
	fs := afero.NewMemMapFs()
	require.NoError(t, afero.WriteFile(fs, "/root/a.change.md", validChangefile("A"), 0644))

	results, err := ReadAllOrFail(context.Background(), fs, "/root", ReadOptions{})
	require.NoError(t, err)
	assert.Len(t, results, 1)
}

// ReadAll is what lets a caller report on every file rather than the first bad one:
// a broken file becomes a Result with an Err, and its neighbors still parse.
func TestReadAll_ReportsEachFileSeparately(t *testing.T) {
	fs := afero.NewMemMapFs()
...
// The same tree read by the two functions: ReadAll carries on, ReadAllOrFail gives up.
func TestReadAllAndReadAllOrFailDifferOnlyInPolicy(t *testing.T) {
	fs := afero.NewMemMapFs()
	...
	_, err = ReadAllOrFail(context.Background(), fs, "/root", ReadOptions{})
	require.Error(t, err)
